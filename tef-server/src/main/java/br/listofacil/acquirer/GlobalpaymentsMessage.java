...
public class GlobalpaymentsMessage {
	...
	private String PIN_POSITION_GP_PINPAD = "317"; // DUKPT 3DES = 3-3DES
													// 17-POSICAO
	...
	public ISOMsg requestLogon(String logicalNumber, long nsu) {
		...
		try {
			...
			Logger.log(new LogEvent(
					"Error: br.listofacil.acquirer.GlobalpaymentsMessage.requestLogon \n " + e.getMessage()));
			e.printStackTrace();
		}
	...
	public ISOMsg requestTable(String logicalNumber, long nsu) {
		...
		try {
			...
			Logger.log(new LogEvent(
					"Error: br.listofacil.acquirer.GlobalpaymentsMessage.requestTable \n " + e.getMessage()));
			e.printStackTrace();
		}
	...
	private String getAcquirerTimestampTables(ISOMsg m) {
		try {
			...
			Logger.log(new LogEvent("Error: br.listofacil.acquirer.GlobalpaymentsMessage.getAcquirerTimestampTables \n "
					+ e.getMessage()));
			e.printStackTrace();
		}
	...
	private ISOMsg requestAcquirer(ISOMsg request) {
		...
		try {
			...
			Logger.log(new LogEvent(
					"Error: br.listofacil.acquirer.GlobalpaymentsMessage.requestAcquirer \n " + e.getMessage()));
			e.printStackTrace();
		}
	...
	public void setResDataInitialization(ISOMsg m) {
		...
		try {
			...
			setTablesInitialization(tab01, tab02, tab03, tab04, tab05, tab06, tab07, tab08, tab12, tab13, tab14, tab15,
					tab16, tab17);

		} catch (Exception e) {
			Logger.log(new LogEvent("Error: br.listofacil.acquirer.GlobalpaymentsMessage.setResDataInitialization \n "
					+ e.getMessage()));
			e.printStackTrace();
		}
	}

	private void setTablesInitialization(String tab01, String tab02, String tab03, String tab04, String tab05,
			String tab06, String tab07, String tab08, String tab12, String tab13, String tab14, String tab15,
	...
	private String getEncryptionRegistry(String value) {
		...
		if (value != null && value.length() > 0) {
			...
			data = cf.convertHexToInt(indexMasterKey) + data + cf.padLeft(String.valueOf(value.length()), 3, "0")
					+ value;
		}
		return data;
	}
	...
	private String getMerchantData(String value) {
		String merchant = "001038" + value.substring(0, 38); // Nome do
																// estabelecimento
		merchant += "002076" + value.substring(38, 114); // Endereço do
															// estabelecimento

		// Na inicializacao da GP nao eh enviado as tags de 003 a 006 e 008 e
		// 009
		...
		merchant += "012001" + listoData.currencyExponent; // Transaction
															// currency exponent
		merchant += "013003" + listoData.terminalCountryCode; // Terminal
																// country code

		merchant += "014008" + cf.stringValueToStringBinary(value.substring(129, 131)); // Byte
																						// 1
		merchant += "015008" + cf.stringValueToStringBinary(value.substring(131, 133)); // Byte
																						// 2
		merchant += "016008" + cf.stringValueToStringBinary(value.substring(133, 135)); // Byte
																						// 3
	...
	private String getProducts(String value) {
		...
		while (value.length() > 0) {
			...
			product += cf.stringValueToStringBinary(value.substring(25, 27)); // Byte
																				// 1
			product += cf.stringValueToStringBinary(value.substring(27, 29)); // Byte
																				// 2
			product += cf.stringValueToStringBinary(value.substring(29, 31)); // Byte
																				// 3
	...
	private String getEmvAid(String value) {
		...
		String registry = paramEMV.IdTable + DATA_POSITION_GP_PINPAD + paramEMV.TableIndex + paramEMV.AIDSize
				+ paramEMV.AID + paramEMV.AppType + paramEMV.Label + paramEMV.AppDefault + paramEMV.AppVersionN1
				+ paramEMV.AppVersionN2 + paramEMV.AppVersionN3 + paramEMV.TerminalCountryCode
				+ paramEMV.TransactionCurrencyCode + paramEMV.TransactionCurrencyExponent + listoData.numeroLogico
				+ paramEMV.MerchantCategoryCode + PARAM_41 + // Terminal number
				paramEMV.TerminalCapabilities + paramEMV.AdditionalTerminalCapabilities + paramEMV.TerminalType
				+ paramEMV.TACDefault + paramEMV.TACDenial + paramEMV.TACOnline + paramEMV.TerminalFloorLimit
				+ paramEMV.TransactionCategoryCode + paramEMV.ActionContactless + paramEMV.TerminalCapabilitiesAID
				+ paramEMV.TerminalContactlessLimit + paramEMV.TerminalContactlessFloorLimit
				+ paramEMV.TerminalCVMRequiredLimit + paramEMV.PaypassMagstripeVersionNumber
				+ paramEMV.SelectContactlessApp + paramEMV.TDOL + paramEMV.DDOL + paramEMV.AuthRespCodeOfflineApproved
				+ paramEMV.AuthRespCodeOfflineDeclined + paramEMV.AuthRespCodeUnableToGoOnlineApproved
				+ paramEMV.AuthRespCodeUnableToGoOnlineDeclined;
		/*
		 * paramEMV.TACContactlessDefault + paramEMV.TACContactlessDenial +
	...
	private BCDataEmvAid getBCDataEmvAid(int id, String value) {
		...
		bcData.AppDefault = "03"; // Padrão da aplicação: “03” - EMV (com ou sem
									// contato)
		bcData.TerminalFloorLimit = "00000000"; // aid.substring(54, 62); -
												// Global solicitou colocar por
												// seguranca 23/06/16 conversa
												// com Lucas Argotechno
		...
		// Na especificacao 0102D a GlobalPayments informa que o tamanho eh em
		// bytes, porem, esta
	...
	private String getPublicKeys(String value) {
		...
		registry = key.IdTable + DATA_POSITION_GP_PINPAD + key.TableIndex + key.RID + key.CAPublicKeyIndex
				+ key.Reserved1 + key.SizeCAPublicKeyExp + key.CAPublicKeyExp + key.SizeCAPublicKeyMod
				+ key.CAPublicKeyMod + key.StatusCheckSum + key.CAPublicKeyCheckSum + key.Reserved2;
	...
	public BCDataPublicKeys getBCDataPublicKeys(int id, String value) {
		...
		data.SizeCAPublicKeyExp = value.substring(14, 15);// value.substring(12,
															// 13);
		...
		data.SizeCAPublicKeyExp = String.valueOf(sizeCAPublicKeyExp / 2); // Uma
																			// posicao
																			// apenas
		...
		data.CAPublicKeyCheckSum = value.substring(19 + sizeCAPublicKey, (40 + 19 + sizeCAPublicKey)); // BC
																										// aceita
																										// 40,
																										// os
																										// outros
		...
		data.Reserved2 = "000000000000000000000000000000000000000000"; // Reservado
																		// uso
																		// futuro
	...
	private ISOMsg getCommonBitsFormatted(ISOMsg msg, long nsu) throws ISOException {
		...
		if (!msg.hasField(7)) {
			isomsg.set(7,
					commonFunctions.padLeft(String.valueOf(trsdate.get(Calendar.MONTH) + 1), 2, "0")
	...
	public TransactionData requestConfirmation(TransactionData requestData) {
		...
		try {
			...
			Logger.log(new LogEvent(
					"Error: br.listofacil.acquirer.GlobalpaymentsMessage.requestConfirmation \n " + e.getMessage()));
			e.printStackTrace();
		}
	...
	public TransactionData requestAdvice(TransactionData requestData) {
		...
		try {
			...
			Logger.log(new LogEvent(
					"Error: br.listofacil.acquirer.GlobalpaymentsMessage.requestAdvice \n " + e.getMessage()));
			e.printStackTrace();
		}
	...
	public TransactionData requestCancellation(TransactionData requestData) {
		...
		try {
			...
			Logger.log(new LogEvent(
					"Error: br.listofacil.acquirer.GlobalpaymentsMessage.requestCancellation \n " + e.getMessage()));
			e.printStackTrace();
		}
	...
	public TransactionData requestUnmaking(TransactionData requestData) {
		...
		try {
			...
			Logger.log(new LogEvent(
					"Error: br.listofacil.acquirer.GlobalpaymentsMessage.requestUnmaking \n " + e.getMessage()));
			e.printStackTrace();
		}
	...
	private ISOMsg getMessage0200(TransactionData requestData) throws ISOException {
		...
		if ((requestData.pan.length() > 0) && (requestData.typeCardRead.equals(ListoData.TRANSACTION_ENTERED))) {
		...
		if ((requestData.expirationDateCard.length() > 0) && (requestData.typeCardRead.equals(ListoData.MAGNETIC)))
		...
		if ((requestData.panSequence.length() > 0) && (Integer.parseInt(requestData.panSequence) > 0))
	...
	private ISOMsg getMessage0220(TransactionData requestData) throws ISOException {
		...
		if ((requestData.responseCode.length() > 0) && (!requestData.responseCode.equals("00")))
	...
	private String getTransactionAdditionalData(String mti, TransactionData data) {
		...
		bit048 += "005" + cf.padLeft(String.valueOf(data.panPartEncrypted.length()), 3, "0") + data.panPartEncrypted;
	...
	private String getTerminalData(String mti, TransactionData data) {
		...
		// Tag=001,Length=008,value=GP + versao_aplicacao +
		// identificacao_listo=LI
		String bit061 = "001008" + VERSAO_ESPEC_GP;

		if (mti.equals(REQ_GP_CONFIRMATION) || mti.equals(REQ_GP_UNMAKING))
			return bit061;

		// verifica se usou o pinpad
		if (data.typeCardRead.equals(ListoData.MAGNETIC) || data.typeCardRead.equals(ListoData.EMV_CONTACT))
		...
		if (data.productDescription.toUpperCase().contains("MASTER")
			...
			bit061 += "003026" + "00000000008" + pointAuthLifeCycle + data.countryCode + pointOfServicePostalCode;
		}

		if ((data.typeCardRead.equals(ListoData.MAGNETIC)) || (data.typeCardRead.equals(ListoData.EMV_CONTACT))) // verifica
																													// se
																													// usou
																													// o
																													// pinpad
	...
	private String getOriginalTransaction(TransactionData data) {
		...
		bit090 = data.originalMessageCode; // Codigo da mensagem original (0100,
											// 0200 ou 0220)
	...
	private String getDataEncrypted(String mti, TransactionData data) {
		...
		// 05 - Presente se solicitado PIN online para validação do portador do
		// cartão.
		...
		if (mti.equals(REQ_GP_PAYMENT)) {
			...
			if (data.typeCardRead.equals(ListoData.EMV_CONTACT)
				...
				bit126 += "0" + especificChar + "    "; // CVV2/CVC2/CID
														// desconsiderado ou nao
														// fornecido (nao eh
														// necessario para EMV)
			else {
	...
	public void requestLogonProcess(ISOMsg message) {
		...
		if (response != null) {
			...
			if (bit48.trim().length() > 0) {
				...
				ListoData listoData = AcquirerSettings.getInitializationTables(ListoData.GLOBAL_PAYMENTS,
						logicalNumber);
				...
				if (listoData != null) {
					if (!versaoTabelas.equals(listoData.versaoTabelasGlobalpayments)) {
						...
						Logger.log(new LogEvent(
								"Diferent tables version - Process of load Globalpayments tables started!"));
